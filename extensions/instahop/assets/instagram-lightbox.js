...
/**
 * Ouvre la lightbox avec gestion du focus et des événements clavier
 * Respecte les standards d'accessibilité WCAG 2.0
 */
window.openLightbox = function (index) {
  ...
  if (lightbox) {
    ...
    // Déplacer le focus vers le premier élément interactif de la lightbox
    const closeButton = document.getElementById("lightbox-close");
    if (closeButton) {
      closeButton.focus();
    }

    // Ajouter le handler clavier si pas déjà présent
...
/**
 * Ferme la lightbox avec restauration du focus
 */
...
/**
 * Gestion du clavier accessible (Escape, Arrow keys)
 */
...
/**
 * Annonce un message aux lecteurs d'écran via ARIA
 */
...
function updateLightbox() {
  ...
  // Construire les stats avec un fallback plus robuste
  let statsHTML = [];
  ...
  if (productsContainer) {
    ...
    if (taggedProducts && taggedProducts.length > 0) {
      let productsHTML =
        '<h4 style="margin: 16px 0 8px 0; font-size: 14px; font-weight: 600; color: #262626;">Produits associés</h4>';
      ...
      taggedProducts.forEach((product, idx) => {
        const price =
          product.price && product.currency
            ...
            : "Prix non disponible";
        productsHTML += `
          <div class="tagged-product-item" role="listitem">
...
/**
 * Fermeture de la lightbox en cliquant sur le fond
 */
